...
.main .block-container{{max-width:1320px;padding-top:.5rem;padding-bottom:3rem;}}
p,label,h1,h2,h3,h4,h5,h6{{color:var(--ink);}}

.ap-topbar{{background:var(--navy);margin:-.5rem -1rem 1.7rem;padding:17px 24px;border-radius:0 0 16px 16px;}}
...
.ap-live{{border:1px solid rgba(255,255,255,.28);border-radius:9px;padding:6px 10px;color:#fff;font-size:12px;}}

.ap-kicker{{color:var(--muted);font-weight:800;text-transform:uppercase;letter-spacing:.14em;font-size:12px;margin-bottom:8px;}}
.ap-title{{color:var(--ink);font-size:36px;line-height:1.05;font-weight:900;letter-spacing:-.04em;margin:0;}}
...
.ap-rule{{background:var(--surface);border:1px solid var(--line);border-left:4px solid #7ca7df;border-radius:12px;padding:12px 14px;color:var(--muted);font-size:13px;line-height:1.5;margin:14px 0 20px;}}
.ap-definition{{background:var(--surface);border:1px solid var(--line);border-radius:14px;padding:14px 16px;margin:14px 0 18px;color:var(--muted);font-size:13px;line-height:1.55;}}
.ap-definition b{{color:var(--ink);}}

.ap-metrics{{display:grid;grid-template-columns:repeat(3,1fr);background:var(--surface);border:1px solid var(--line);border-radius:16px;overflow:hidden;margin:20px 0 28px;box-shadow:0 8px 22px var(--shadow);}}
.ap-metric{{display:flex;align-items:center;gap:16px;padding:22px 26px;border-right:1px solid var(--line);min-height:86px;}}
...
.ap-section-head{{display:flex;justify-content:space-between;align-items:center;gap:16px;margin:26px 0 12px;}}
.ap-section-title{{font-size:20px;font-weight:900;color:var(--ink);}}
.ap-section-count{{font-size:13px;color:var(--muted);}}

.ap-card{{background:var(--surface);border:1px solid var(--line);border-radius:16px;padding:18px 20px;margin:12px 0 6px;box-shadow:0 8px 22px var(--shadow);}}
...
.ap-card-right{{min-width:150px;text-align:right;color:var(--amber);font-size:13px;white-space:nowrap;}}
.ap-card-block{{margin-top:13px;padding-top:12px;border-top:1px solid var(--line);}}
.ap-card-label{{color:var(--muted);text-transform:uppercase;letter-spacing:.08em;font-size:10px;font-weight:800;margin-bottom:5px;}}
...
/* SELECTS: zera o tema nativo escuro do Streamlit no modo claro */
...
/* Inputs/textarea */
.stTextArea textarea,.stTextInput input{{background:var(--input)!important;color:var(--input-text)!important;-webkit-text-fill-color:var(--input-text)!important;border-color:var(--line)!important;}}
.stTextArea textarea::placeholder,.stTextInput input::placeholder{{color:var(--muted)!important;opacity:.8!important;}}

/* BOTÕES: texto sempre visível */
div[data-testid="stButton"] > button{{background:var(--surface)!important;color:var(--ink)!important;border:1px solid var(--line)!important;border-radius:11px!important;box-shadow:none!important;font-weight:750!important;opacity:1!important;}}
...
/* Primário para ações operacionais */
...
div[data-testid="stRadio"] label{{background:var(--surface)!important;border:1px solid var(--line)!important;border-radius:11px!important;min-height:46px!important;padding:0 12px!important;display:flex!important;align-items:center!important;justify-content:center!important;}}
div[data-testid="stRadio"] label p,div[data-testid="stRadio"] label span{{color:var(--ink)!important;-webkit-text-fill-color:var(--ink)!important;opacity:1!important;font-size:13px!important;font-weight:750!important;}}
...
.ap-theme-caption{{text-align:right;color:var(--muted);font-size:10px;margin-bottom:-5px;}}
...
div[data-testid="stExpander"]{{background:var(--surface)!important;border:1px solid var(--line)!important;border-radius:13px!important;}}
div[data-testid="stExpander"] summary,div[data-testid="stExpander"] summary *{{color:var(--ink)!important;-webkit-text-fill-color:var(--ink)!important;}}
...
def parse_payload(raw: Any) -> dict[str, Any]:
    ...
    try:
        ...
        # compatibilidade se houver texto antigo no campo
        text = str(raw).strip()
...
def render_metrics(conferir: int, cobrar: int, elaborar: int) -> None:
    st.markdown(
        ...
        <div class="ap-metrics">
          <div class="ap-metric"><div class="ap-metric-num orange">{conferir:02d}</div><div class="ap-metric-label">Para conferir</div></div>
          <div class="ap-metric"><div class="ap-metric-num blue">{cobrar:02d}</div><div class="ap-metric-label">Aguardando resposta</div></div>
          <div class="ap-metric"><div class="ap-metric-num green">{elaborar:02d}</div><div class="ap-metric-label">Para elaborar</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_card_summary(row: pd.Series, state: dict[str, Any] | None) -> None:
    ...
    if state:
        ...
        status = str(state.get("review_status") or "")
        if status == "accepted":
            who = safe_text(state.get("accepted_by"), "Orçamentos")
            saved_html = f'<div class="ap-saved"><b>Decisão do Orçamentos:</b> levantamento liberado por {escape(who)}.</div>'
        elif items:
            saved_html = '<div class="ap-saved"><b>Pendências definidas pelo Orçamentos:</b> ' + escape("; ".join(map(str, items))) + '</div>'

    gaps_html = "" if gaps == "—" else f'<div class="ap-card-block"><div class="ap-card-label">Sugestão automática de lacunas</div><div class="ap-card-text">{escape(gaps)}</div></div>'
    reply_html = "" if last_reply == "—" else f'<div class="ap-card-block"><div class="ap-card-label">Último retorno da Engenharia</div><div class="ap-card-text">{escape(last_reply)}</div></div>'
        ...
        <div class="ap-card">
          <div class="ap-card-top">
            <div style="flex:1;min-width:0;">
              <div class="ap-badges"><span class="ap-badge {fila_badge(fila)}">{escape(fila)}</span><span class="ap-ref">REF. {escape(ref)}</span></div>
              <div class="ap-card-title">{escape(title)}</div>
              <div class="ap-card-meta"><b>Unidade:</b> {escape(unidade)} &nbsp;•&nbsp; <b>Supervisor:</b> {escape(supervisor)} &nbsp;•&nbsp; <b>Etapa Trello:</b> {escape(etapa)}</div>
            </div>
            <div class="ap-card-right">{escape(prazo)}</div>
          </div>
          <div class="ap-card-block"><div class="ap-card-label">Próximo passo atual</div><div class="ap-card-text">{escape(next_action)}</div></div>
          {gaps_html}{reply_html}{saved_html}
          <div class="ap-card-block"><div class="ap-card-text"><b>Aguardando:</b> {escape(waiting)} &nbsp;•&nbsp; <b>Serviço:</b> {escape(service)} &nbsp;•&nbsp; {link_html}</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
# Esta tela é a CENTRAL DE LEVANTAMENTOS. Não mistura orçamento já enviado/execução.
levantamento_stages = {
    "SOLICITADOS",
...
head_left, head_right = st.columns([8, 1.15])
with head_left:
    ...
    st.markdown('<div class="ap-subtitle">O painel não serve só para mostrar o Trello: aqui Orçamentos define exatamente o que falta e transforma isso em uma cobrança objetiva.</div>', unsafe_allow_html=True)
with head_right:
    st.markdown('<div class="ap-theme-caption">Aparência</div>', unsafe_allow_html=True)
    st.toggle("🌙 Escuro", key="dark_mode", help="Alternar modo claro/escuro")

ctl1, ctl2, ctl3 = st.columns([1.1, 2.2, 5])
with ctl1:
...
with ctl3:
    ...
    db_status = "Neon conectado" if db_available() else "Neon indisponível — ações não serão salvas"
    st.markdown(f'<div class="ap-sync">Quadro: <b>{escape(board_name)}</b> • {escape(db_status)}</div>', unsafe_allow_html=True)

st.markdown(
    """
    <div class="ap-definition">
      <b>Definição desta tela:</b> o Trello informa o que existe no card; o <b>Orçamentos decide o que precisa ser cobrado</b>.
      Em cada demanda você pode abrir <b>“Definir o que solicitar”</b>, revisar as sugestões do sistema, escrever os itens exatos que o supervisor precisa responder e salvar.
      Essa definição fica no Neon e será exatamente a lista que o portal do engenheiro exibirá quando construirmos a página dele.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with f1: filtro_eng = st.selectbox("Engenharia", engineer_options)
with f2: filtro_stage = st.selectbox("Etapa Trello", stage_options)
with f3: filtro_wait = st.selectbox("Aguardando", wait_options)
with f4: filtro_prazo = st.selectbox("Prazo", ["Todos", "Atrasados", "Hoje", "Até 2 dias", "Sem prazo"])

filtered = df.copy()
if not filtered.empty:
    if filtro_eng != "Todos": filtered = filtered[filtered["Engenharia"] == filtro_eng]
    if filtro_stage != "Todas": filtered = filtered[filtered["Etapa Trello"] == filtro_stage]
    if filtro_wait != "Todos": filtered = filtered[filtered["Aguardando"] == filtro_wait]
    dias = pd.to_numeric(filtered.get("Dias até prazo", pd.Series(index=filtered.index, dtype=float)), errors="coerce")
    if filtro_prazo == "Atrasados": filtered = filtered[dias < 0]
    elif filtro_prazo == "Hoje": filtered = filtered[dias == 0]
    elif filtro_prazo == "Até 2 dias": filtered = filtered[dias.between(0, 2, inclusive="both")]
    elif filtro_prazo == "Sem prazo": filtered = filtered[dias.isna()]
...
st.markdown(f'<div class="ap-section-head"><div class="ap-section-title">Fila de levantamentos</div><div class="ap-section-count">{len(filtered)} demandas</div></div>', unsafe_allow_html=True)
...
if current_queue not in queue_order: current_queue = "Cobrar Engenharia"
selected_label = st.radio("Fila", [queue_labels[q] for q in queue_order], index=queue_order.index(current_queue), horizontal=True, label_visibility="collapsed")
...
if queue_df.empty:
    ...
    for _, row in queue_df.iterrows():
        ...
        with st.expander("✎ Definir o que solicitar / decidir próxima ação"):
            ...
            st.caption("A sugestão automática é só um ponto de partida. Apague, corrija e acrescente o que realmente é necessário para conseguir orçar.")
            ...
            with o1:
                owner_label = st.selectbox("Quem precisa responder?", list(owner_labels.keys()), index=owner_index, key=f"owner_{card_id}")
            with o2:
                note = st.text_input("Observação interna (opcional)", value=str(payload.get("note") or ""), key=f"note_{card_id}", placeholder="Ex.: confirmar também com fornecedor de esquadria")

            b1, b2, b3 = st.columns([1.6, 1.4, 1])
            with b1:
                if st.button("Salvar pendências", type="primary", use_container_width=True, key=f"save_{card_id}"):
                    ...
                    if ok: st.rerun()
            with b2:
                if st.button("✓ Pronto para elaborar", use_container_width=True, key=f"ready_{card_id}"):
            ...
            if owner_labels[owner_label] == "engineering":
                st.caption("Ao salvar, esta lista passa a ser a definição oficial do que o supervisor deve responder. O portal do engenheiro será construído depois lendo exatamente estes itens.")
            else:
                st.caption("Ao salvar, a demanda passa a ser tratada como pendência externa, sem atribuir o atraso à Engenharia.")

with st.expander("Ver tabela de conferência"):
    cols = ["Demanda", "Unidade", "Engenharia", "Fila", "Etapa Trello", "Aguardando", "Pendência / próxima ação", "Situação do prazo", "Data visita"]
