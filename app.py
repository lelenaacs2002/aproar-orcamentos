...
def load_snapshot(force_nonce: int = 0) -> dict[str, Any]:
    ...
    board = secret("TRELLO_BOARD", "TX8hGvmI")
    client = TrelloClient(api_key=api_key, token=token, board=board)
...
with st.sidebar:
    ...
    if st.button("🔄 Atualizar Trello", use_container_width=True):
        ...
        st.rerun()
    st.caption("Leitura automática em cache por até 2 minutos.")
    st.divider()
...
try:
    ...
    render_header()
    st.error(str(exc))
    st.info("Crie os Secrets do Streamlit usando o arquivo `.streamlit/secrets.toml.example` deste projeto. As credenciais nunca devem ser enviadas ao GitHub.")
    st.stop()
